...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['picName']) && !empty(trim($_POST['picName']))) {
        ...
        header("Location: newbookpicN9.php");
        exit();
    }
}
?>
...
<body>
    <div class="form-box">
        ...
        <form action="createpicNameN9.php" method="post">
            <input type="text" name="picName" placeholder="Enter PIC Name" required />

            <div>
                <button type="submit" class="confirm">Confirm</button>
                <button type="button" class="cancel" onclick="window.location.href='newbookpicN9.php'">Cancel</button>
            </div>
        </form>
    </div>
</body>
